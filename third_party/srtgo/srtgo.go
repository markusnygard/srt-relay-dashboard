...
// SockState reports the current SRT socket state. Values other than
// SRTS_CONNECTED mean the peer is gone (BROKEN/CLOSING/CLOSED), which bstats
// alone cannot detect.
func (s SrtSocket) SockState() int {
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()
	return int(C.srt_getsockstate(s.socket))
}
